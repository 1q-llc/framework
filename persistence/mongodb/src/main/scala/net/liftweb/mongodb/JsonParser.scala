...
object JsonParser extends SimpleInjector {
  /**
    * Set this to override JsonParser turning strings that are valid
    * ObjectIds into actual ObjectIds. For example, place the following in Boot.boot:
    *
    * <code>JsonParser.stringProcessor.default.set((s: String) => BsonString(s))</code>
    */
  ...
  */
  def serialize(a: Any)(implicit formats: Formats): JValue = {
    ...
    a.asInstanceOf[AnyRef] match {
      ...
      case x => {
        // println("Unknown Bson type: "+x.getClass)
        JNothing
      }
    }
  }
