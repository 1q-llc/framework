...
object MongoIndex {
  ...
  def apply(keys: JObject)(implicit formats: Formats): MongoIndex = MongoIndex(JsonParser.parse(keys), new IndexOptions)
  def apply(keys: JObject, unique: Boolean)(implicit formats: Formats): MongoIndex = MongoIndex(JsonParser.parse(keys), (new IndexOptions).unique(unique))
  def apply(keys: JObject, opts: IndexOptions)(implicit formats: Formats): MongoIndex = MongoIndex(JsonParser.parse(keys), opts)
}
...
object MongoDB {
  ...
  // for legacy purposes
  @deprecated("Use getCollection instead", "3.3.1")
