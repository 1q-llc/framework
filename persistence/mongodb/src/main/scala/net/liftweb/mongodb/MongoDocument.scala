...
import org.bson.codecs.{DecoderContext, UuidCodecProvider}
...
import com.mongodb.client.model.{IndexOptions, InsertOneOptions, ReplaceOptions, UpdateOptions}
...
trait MongoDocumentMeta[BaseDocument] extends JsonObjectMeta[BaseDocument] with MongoMeta[BaseDocument, BsonDocument] {
  ...
  def indexes: Seq[MongoIndex] = Nil
  ...
  def useCollection[T](f: MongoCollection[BsonDocument] => T): T =
    MongoDB.useMongoCollection(connectionIdentifier, collectionName, classOf[BsonDocument])(mc => f(mc.withCodecRegistry(codecRegistry)))

  def useCollection[T](db: MongoDatabase)(f: MongoCollection[BsonDocument] => T): T = {
    val mc = db.getCollection(collectionName, classOf[BsonDocument])
    f(mc.withCodecRegistry(codecRegistry))
  }
   ...
   */
  def useDatabase[T](f: MongoDatabase => T): T = MongoDB.useDatabase(connectionIdentifier)(md => f(md.withCodecRegistry(codecRegistry)))
  ...
  def create(dbo: Bson): BaseDocument = {
    val jv = JsonParser.serialize(dbo)
   ...
   */
  def find(json: JObject): Option[BaseDocument] = find(JsonParser.parse(json))
  ...
  def insertOne(inst: BaseDocument, opts: InsertOneOptions = new InsertOneOptions): Box[BaseDocument] = tryo {
    useCollection { coll =>
      val bson = JsonParser.parse(toJObject(inst))
  ...
  def replaceOne(qry: JObject, inst: BaseDocument, opts: ReplaceOptions): Box[UpdateResult] = tryo {
    useCollection { coll =>
      val bson = JsonParser.parse(toJObject(inst))
      coll.replaceOne(JsonParser.parse(qry), bson, opts)
    }
  }
  ...
  @deprecated("Use updateOne, updateMany, or replaceOne instead", "3.3.1")
